...
Plugin ID: `com.snagar.quicknotes` · Version: `1.0.0` · License: [MIT](LICENSE)
...
<p align="center">
  <img src="marketing/quicknotes-list.png" alt="Notes list" width="420">
  <img src="marketing/quicknotes-editor.png" alt="Legal-pad editor" width="420">
</p>
...
1. Build (see [Build](#build)) or download `notes-plugin-1.0.0.zip` from `build/distributions/`.
2. In the IDE: **Settings → Plugins → ⚙ → Install Plugin from Disk…**
3. Select the zip and restart when prompted.
4. Open **View → Tool Windows → Quick Notes** (right dock by default).
...
Shortcuts apply **only while the Quick Notes tool window is focused**, so they do not clash with the rest of the IDE. Remap them under **Settings → Keymap → Plug-ins → Quick Notes**.
...
Output zip: `build/distributions/notes-plugin-1.0.0.zip`
...
This launches a throwaway IDE with Quick Notes loaded. It does not change your real Android Studio / IntelliJ install.
...
```
src/main/kotlin/com/snagar/quicknotes/
  model/      Note, SortKey
...
The plugin id `com.snagar.quicknotes` is **permanent** after the first upload. Do not change it.
